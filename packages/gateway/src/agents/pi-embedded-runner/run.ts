...
import { afterLLMCall, getCostOptimizer } from "../../cost-optimizer/runtime-hooks.js";
...
export async function runEmbeddedPiAgent(
  ...
  return enqueueSession(() =>
    enqueueGlobal(async () => {
      ...
      // Cost Optimizer: advisory model-routing (Session 3 — log only, no override)
      try {
        const costOpt = getCostOptimizer();
        const preResult = await costOpt.preRequest("local", params.prompt);
        if (preResult.allowed && preResult.model !== modelId) {
          log.info(
            `[cost-optimizer] advisory: recommended model=${preResult.model} (current=${provider}/${modelId})`,
          );
        }
      } catch {
        // Non-blocking — cost optimizer errors never stop the agent run
      }
      ...
      try {
        while (true) {
          ...
          // Cost Optimizer: usage-tracking (Session 3 — fire-and-forget)
          if (attemptUsage) {
            afterLLMCall({
              model: modelId,
              tier: "ROUTINE",
              ...
              cachedTokens: attemptUsage.cacheRead ?? 0,
              error: !!attempt.promptError,
              toolName: attempt.lastToolError?.toolName,
