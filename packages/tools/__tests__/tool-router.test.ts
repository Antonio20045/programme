...
describe('ConfirmationManager', () => {
  ...
  it('skips confirmation when no session or emitter is set', async () => {
  ...
  it('clears active session', () => {
    ...
    // With no active session, should skip confirmation
    const emitFn = vi.fn()
    manager.setEmitter(emitFn)

    void manager.requestConfirmation('tc-4', 'shell', {}).then((d) => {
